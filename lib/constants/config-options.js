...
var Args = require('yargs').usage('$0 --help | --show-config [OPTIONS] | [OPTIONS]').options({
  ...
  'graphite.dsn': {
    'describe': 'DSN for graphite connection',
    'default': '',
    'requiresArg': true
  },
  'graphite.prefix': {
    'describe': 'Prefix for graphite metrix',
    'default': '',
    'requiresArg': true
  },
  ...
  ['http'].forEach((k) => {
    ...
    if (v !== false) {
      ...
      if (m) {
        ...
        err.push(`Incorrect value of listen.$ {
          k
        }
        parameter`);
      }
...
function applyOptions(opts, defaults) {
  if (!(
    opts &&
    typeof(opts) == "object" &&
    defaults &&
      ...
      if (defaults.hasOwnProperty(k)) {
        if (typeof(opts[k]) == "object") {
          if (opts[k] instanceof Array) {
            ...
            applyOptions(opts[k], defaults[k]);
          }
        } else {
          defaults[k] = opts[k];
        }
...
exports.defaults = applyOptions(Argv, defaults);